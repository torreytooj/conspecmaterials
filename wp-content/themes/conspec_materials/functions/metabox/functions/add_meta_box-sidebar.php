...
function add_sidebar_meta_box() {
    add_meta_box(
		'sidebar', // $id
		__('Sidebar', GETTEXT_DOMAIN), // $title 
...
add_action('admin_head','add_sidebar_scripts');
function add_sidebar_scripts() {
	global $sidebar_meta_fields, $post, $pagenow;
	if ( $pagenow == 'post-new.php' || $pagenow == 'post.php' ) {
	
	$output = '<script type="text/javascript">
				jQuery(function() {';
	
	foreach ($sidebar_meta_fields as $field) { // loop through the fields looking for certain types
		// date
		if($field['type'] == 'date')
...
if( getCurrentTemplate == "template-full-width-blog.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else if ( getCurrentTemplate == "template-portfolio-2.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else if ( getCurrentTemplate == "template-portfolio-3.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else if ( getCurrentTemplate == "template-portfolio-4.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else if ( getCurrentTemplate == "template-portfolio-6.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else if ( getCurrentTemplate == "template-full-width.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else if ( getCurrentTemplate == "template-front.php"||getCurrentTemplate == "template-front-blog.php"){
    $("#sidebar-hide").attr("checked", false);
    $("#sidebar").css({"display":"none"});
}else{
    $("#sidebar-hide").attr("checked", true);
...
function save_sidebar_meta($post_id) {
	...
	// verify nonce
	$nonce = '';
	if(isset($_POST['sidebar_meta_box_nonce'])){ $nonce = $_POST['sidebar_meta_box_nonce']; }
