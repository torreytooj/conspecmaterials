...
<?php $post_header_image_thumbnail = wp_get_attachment_image_src( get_post_meta($post->ID, 'portfolio_header_image', true), 'large' );?>
...
<?php $term = array_pop($terms); ?>
        ...
        <?php if(!$box_caption||($box_caption&&has_post_thumbnail())||($box_caption&&$video_thumb)||($box_caption&&$post_header_image_thumbnail)){?><div class="<?php $allClasses = get_post_class(); foreach ($allClasses as $class) { echo $class . " "; } ?>box" data-category="<?php echo $term->name; ?>"><?php }?> 
	...
	<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) {?>
            ...
            <div class="box-image">
                ...
                <div class="lightbox-text">
                        <?php the_title();?> <span><?php _e('In', GETTEXT_DOMAIN);?> <?php $category = get_the_category(); echo $term->name; ?></span>
                </div>
            </div>
	...
	<?php }?>
            ...
            <div class="box-caption <?php if(!has_post_thumbnail()&&!$post_header_image_thumbnail&&!$video){?>no-caret<?php }?>">
                ...
                <div class="box-caption-bottom">
                    ...
                    <div class="mb_social">
                        <a href="http://www.facebook.com/sharer/sharer.php?s=100&p[url]=<?php the_permalink();?>&p[images][0]=<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'medium' ); echo $image[0];?>&p[title]=<?php the_title();?>&p[summary]=<?php echo lpd_excerpt(25)?>" class="fb-icon"></a>
                        ...
                        <a href="http://pinterest.com/pin/create/button/?url=<?php the_permalink();?>&media=<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'medium' ); echo $image[0];?>&description=<?php echo lpd_excerpt(25)?>" class="pi-icon"></a>
                    </div>
                </div> 
                <?php }?>   
            </div>
