...
class WC_Cart_Notices_Admin {
	...
	/** @var \SV_WP_Admin_Message_Handler */
	...
	private function validate_options() {
		...
		if ( 'cart_notice_new' == $_POST['action'] ) {
			if ( ! $_POST['notice_type'] || ! isset( $this->notice_types[ $_POST['notice_type'] ] ) ) {
				$this->message_handler->add_error( __( 'You must choose a Notice Type', WC_Cart_Notices::TEXT_DOMAIN ) );
			}
		}

		// notice name is required
		if ( ! $_POST['notice_name'] ) {
			$this->message_handler->add_error( __( 'You must provide a Notice Name', WC_Cart_Notices::TEXT_DOMAIN ) );
		}
		...
		if ( $wpdb->get_var( $name_exists_query ) ) {
			$this->message_handler->add_error( __( 'That name is already in use', WC_Cart_Notices::TEXT_DOMAIN ) );
		}
		...
		if ( $_POST['minimum_order_amount'] && ( ! is_numeric( $_POST['minimum_order_amount'] ) || (float) $_POST['minimum_order_amount'] < 0 ) ) {
			$this->message_handler->add_error( __( 'Minimum order amount must be positive number, or empty', WC_Cart_Notices::TEXT_DOMAIN ) );
		}
		...
		if ( $_POST['threshold_order_amount'] && ( ! is_numeric( $_POST['threshold_order_amount'] ) || (float) $_POST['threshold_order_amount'] < 0 ) ) {
			$this->message_handler->add_error( __( 'Threshold amount must be positive number, or empty', WC_Cart_Notices::TEXT_DOMAIN ) );
		}
		...
		if ( $_POST['deadline_hour'] && ( ! is_numeric( $_POST['deadline_hour'] ) || (int) $_POST['deadline_hour'] < 1 || (int) $_POST['deadline_hour'] > 24 ) ) {
			$this->message_handler->add_error( __( 'Deadline hour must be in 24-hour format, between 1 to 24', WC_Cart_Notices::TEXT_DOMAIN ) );
		}
	}
	...
	private function handle_create_update_cart_notice( $action ) {
		...
		switch ( $notice_type ) {
			...
			case 'products':
				$fields['data']['product_ids']        = SV_WC_Plugin_Compatibility::is_wc_version_gte_2_3() ? explode( ',', $this->get_request( 'product_ids' ) ) : $this->get_request( 'product_ids' );
			...
			break;
			case 'categories':
				$fields['data']['category_ids'] = SV_WC_Plugin_Compatibility::is_wc_version_gte_2_3() ? explode( ',', $this->get_request( 'category_ids' ) ) : $this->get_request( 'category_ids' );
			break;
		}
	...
	public function wc_cart_notices_options() {
		...
		require_once( wc_cart_notices()->get_plugin_path() .  '/admin/admin-options.php' );
	}
	...
	public function add_global_settings( $settings ) {
		...
		$cart_notices_settings = apply_filters( 'wc_cart_notices_global_settings', array(
			...
			array(
				'title'   => __( 'Delete data', WC_Cart_Notices::TEXT_DOMAIN ),
				'desc'    => __( 'Delete all Cart Notices data on uninstall', WC_Cart_Notices::TEXT_DOMAIN ),
