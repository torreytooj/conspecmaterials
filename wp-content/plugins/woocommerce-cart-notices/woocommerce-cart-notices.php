...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.0.0', __( 'WooCommerce Cart Notices', 'woocommerce-cart-notices' ), __FILE__, 'init_woocommerce_cart_notices', array( 'minimum_wc_version' => '2.2', 'backwards_compatible' => '4.0.0' ) );
...
class WC_Cart_Notices extends SV_WC_Plugin {
	...
	private function admin_includes() {
		...
		require_once( $this->get_plugin_path() . '/classes/class-wc-cart-notices-admin.php' );
	...
	private function get_cart_total() {
		...
		if ( ! WC()->cart->prices_include_tax ) {
			...
			$cart_contents_total = WC()->cart->subtotal_ex_tax;
		} else {
			// if prices do include tax, add the tax amount back in
			$cart_contents_total = WC()->cart->subtotal;
		}
	...
	public function get_referer_notice( $notice ) {
		...
		// get the referer
		if ( ! WC()->session->wc_cart_notice_referer ) return false;
	...
	public function get_products_notice( $notice ) {
		...
		if ( empty( WC()->cart->cart_contents ) ) return false;
	...
	public function get_notices() {
		...
		if ( ! $this->notices ) {
			...
			$wpdb->hide_errors(); // avoid database error when plugin is first installed
	...
	public function get_plugin_name() {
		return __( 'WooCommerce Cart Notices', self::TEXT_DOMAIN );
	}
 ...
 */
$GLOBALS['wc_cart_notices'] = wc_cart_notices();
