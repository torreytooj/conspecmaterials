...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
    function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
        ...
        global $wpdb, $codenegar_aas;

        if ( empty( $url ) )
            return new WP_Error( 'no_image_url', __( 'No image URL has been entered.', $codenegar_aas->text_domain ), $url );
	...
	$retina = $retina ? ( $retina === true ? 2 : $retina ) : 1;
        ...
        if ( !file_exists( $dest_file_name ) ) {
            ...
            if ( !$size )
                return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".', $codenegar_aas->text_domain), $file_path );
            ...
            if ( IMAGETYPE_GIF == $orig_type ) {
                if ( !imagegif( $new_image, $dest_file_name ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)', $codenegar_aas->text_domain ) );
            }
            elseif ( IMAGETYPE_PNG == $orig_type ) {
                if ( !imagepng( $new_image, $dest_file_name ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).', $codenegar_aas->text_domain ) );
            }
            else {
                ...
                if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).', $codenegar_aas->text_domain ) );
            ...
            if ( !$new_size )
                return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize', $codenegar_aas->text_domain ), $dest_file_name );
