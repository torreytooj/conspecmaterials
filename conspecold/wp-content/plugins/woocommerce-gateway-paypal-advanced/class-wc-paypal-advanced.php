...
class WC_Paypal_Advanced extends WC_Payment_Gateway {
	...
	public function relay_response() {
		...
		if ( $this->debug == 'yes' )
			$this->log->add( 'paypal_advanced', sprintf( __( 'Relay Response Order Number = %s', 'wc_paypaladv' ), $_POST['ORDERID'] ) );
		...
		// Log
			if ($not_silentreq_debug) $this->log->add( 'paypal_advanced', sprintf( __('Redirecting to Thank You Page for order %s', 'wc_paypaladv'), $order->get_order_number()));
		...
		if ( isset($_REQUEST['error']) && $_REQUEST['error'] == 'true' && $_POST['RESULT'] != 0 ) { //handle errors and declines
			...
			if($_POST['RESULT']==12 && $status!='failed') {
				...
				// Log the status
				if ($debug == 'yes') $this->log->add( 'paypal_advanced', sprintf( __('Status has been changed to failed for order %s', 'wc_paypaladv'), $order->get_order_number()));
			}
			...
			if ($not_silentreq_debug)
				$this->log->add( 'paypal_advanced', sprintf( __('Silent Error Occurred while processing %s : %s, status: %s', 'wc_paypaladv'), $order->get_order_number(),urldecode($_POST['RESPMSG']),$_POST['RESULT']));
			elseif($debug == 'yes')
				$this->log->add( 'paypal_advanced', sprintf( __('Error Occurred while processing %s : %s, status: %s', 'wc_paypaladv'), $order->get_order_number(),urldecode($_POST['RESPMSG']),$_POST['RESULT']));
		...
		// Add order note
		$order->add_order_note( sprintf( __('PayPal Advanced payment completed (Order: %s). Transaction number/ID: %s. But needs to Inquiry transaction to have confirmation that it is actually paid.', 'wc_paypaladv' ), $order->get_order_number(),$_POST['PNREF'] ) );
		...
		//inquire transaction, whether it is really paid or not
			...
			if ($inquiry_result_arr['RESULT'] == 0) {//if approved
				...
				// Add order note
				$order->add_order_note( sprintf( __('Received result of Inquiry Transaction for the  (Order: %s) and is successful', 'wc_paypaladv' ), $order->get_order_number() ) );
	...
	function get_secure_token( $order ) {
		...
		//override the layout with mobile template, if browsed from mobile
		...
		try {
			...
			if ($this->debug=='yes'){
				...
				$this->log->add( 'paypal_advanced', sprintf(__('Requesting for the Secured Token for the order %s with following URL and Paramaters: %s', 'wc_paypaladv'), $order->get_order_number(),$this->hostaddr.'?'.$logData));
			}
		...
		// Handle response
			...
			if ($this->debug=='yes') $this->log->add( 'paypal_advanced', sprintf(__('Secured Token generation failed for the order %s with error: %s', 'wc_paypaladv'), $order->get_order_number(), $e->getMessage() ) );
...
?>
	...
	public function process_payment( $order_id ) {
		...
		try {
			...
			if ($this->securetoken !="") {
				...
				//Log
				if ($this->debug=='yes') $this->log->add( 'paypal_advanced', sprintf(__('Secured Token generated successfully for the order %s', 'wc_paypaladv'), $order->get_order_number()));
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if (!is_null( $amount ) && $order->get_total() > $amount) {
			...
			//Log
			if ($this->debug=='yes') {
				$this->log->add( 'paypal_advanced', sprintf( __( 'Response of the refund transaction: %s', 'wc_paypaladv' ), print_r( $refund_result_arr, true ) ) );
			}
	 ...
	 **/
	 public function receipt_page($order_id) {
		...
		if ($this->layout == 'MINLAYOUT' || $this->layout == 'C') {
			...
			//Log
			if ($this->debug=='yes') $this->log->add( 'paypal_advanced', sprintf(__('Show payment form(IFRAME) for the order %s as it is configured to use Layout C', 'wc_paypaladv'), $order->get_order_number()));
...
?>
		...
		<?php
			...
			//Log
			if ($this->debug=='yes') $this->log->add( 'paypal_advanced', sprintf(__('Show payment form redirecting to '.$location.' for the order %s as it is not configured to use Layout C', 'wc_paypaladv'), $order->get_order_number()));
...
function add_paypal_adv_gateway( $methods ) {
	$methods[] = 'WC_Paypal_Advanced'; return $methods;
}
